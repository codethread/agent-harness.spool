...
- **DELTA-DaemonRuntime-001.CC3:** Runtime config reload mirrors startup layering exactly. It clears the same weaver-lifetime registries as today, reinstalls built-ins, restarts event dispatch, then reloads `init.clj` and `init.local.clj` in order when present.
