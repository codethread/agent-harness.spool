...
**Builds on (post-merge baseline):** [attr-scaling-ship-now](../attr-scaling-ship-now/proposal.md) and its spec deltas `ASSN-DELTA-001/002/003`. Implementation begins only after that feature squash-merges to `main`; every spec delta here is written against the **post-merge** root specs, not today's.
...
- **PROP-EavAttrStorage-001.G3 — Uniform query capability.** Every key — with no declaration, no threshold, no hot/cold split visible to the query author — keeps full predicate capability (`:=`, `:!=`, `:<`/`:<=`/`:>`/`:>=`, `:in`, `:exists`, `:missing`, logical composition, cross-key). No key is ever less capable or slower than it is today. The L0b declared-key registry problem **dissolves**: rows make every non-archived key first-class and indexable by construction.
...
**Hard acceptance invariant (governs every goal):** the attribute-map contract at `skein.api.*` is **byte-identical on the wire** to today's, and no key is ever less capable or slower than today. The storage change is invisible above `skein.core.*`.
...
- **PROP-EavAttrStorage-001.S1 (EAV storage):** Add an `attributes` table shaped roughly `(strand_id TEXT REFERENCES strands(id) ON DELETE CASCADE, key TEXT, value TEXT /* JSON-encoded per value */, archived INTEGER NOT NULL DEFAULT 0, PRIMARY KEY (strand_id, key))`. The primary key covers per-strand assembly (a `strand_id`-prefixed read). A `(key, value)` **partial** index `WHERE archived = 0` makes every non-archived key uniformly filterable and indexable. The `strands.attributes` document column is removed; per-value JSON encoding replaces the whole-document `json_valid` CHECK. `ON DELETE CASCADE` keeps burn/supersede/batch lifecycle automatic (as the document column did, and as `DESIGN.md` (4) proved for the overflow-table variant).
- **PROP-EavAttrStorage-001.S2 (Archive tier as a flag, not a table):** The archive tier is the `archived` flag plus the `WHERE archived = 0` partial indexes — **not** a second table. Hot query paths structurally cannot see archived rows: the partial index and an `archived = 0` predicate on the assembly/scan paths exclude them. Full-fidelity point reads (`show`) transparently include archived values (correct-but-slower on cold data is acceptable — TEN-003 forbids *wrong*, not *slow*). Core ships trusted primitives `archive!` / `unarchive!` operating per strand and per (strand, keys); archive **policy** is userland (`PROP-EavAttrStorage-001.NG4`).
- **PROP-EavAttrStorage-001.S3 (Patch semantics preserved exactly):** `update-strand!` keeps today's `json_patch`-equivalent contract at the API: per-key merge, a `null` value deletes the key (now a row delete rather than a `json_patch` null-removal), the whole patch is one transaction, and whole-map replace is still supported. The whole-document `json_valid` CHECK becomes application-level validation via the existing `::specs/attributes` spec (`src/skein/core/specs.clj`), preserving fail-loud rejection of non-JSON-object-encodable attribute maps.
...
- **PROP-EavAttrStorage-001.S10 (Sequencing):** Implementation builds off `main` **after** `attr-scaling-ship-now` squash-merges. This proposal may proceed immediately; the spec deltas must be written against the **post-merge** root specs (that feature's task 4 merges `ASSN-DELTA-001/002/003` into `strand-model.md` / `cli.md` / `daemon-runtime.md`). The devflow run should **hold at the `human-signoff-proposal` checkpoint until ship-now lands**, then rebase this worktree onto `main` before spec work begins.
...
- **Decision:** When assembling attribute rows back into the wire JSON object, emit keys in a **deterministic order** (recommended: lexicographic by key). The wire value stays a JSON object; JSON object key order is not semantically significant, and consumers must not depend on a particular order beyond it being deterministic.
...
- **PROP-EavAttrStorage-001.Q2:** The `archive!` / `unarchive!` op names and their trusted-surface tier (Clojure config/REPL only, mirroring `declare-acyclic-relation!`), plus whether they take strand-id + optional key set.
