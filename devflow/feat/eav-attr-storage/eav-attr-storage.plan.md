...
| EAS-PLAN-001.AA4 | `src/skein/api/weaver/alpha.clj` | Add trusted `archive!`/`unarchive!`/migrate ops; keep in-process reads full-fidelity over the assembled map (archived included). **Remove** the declare/list indexed-attr-key ops. No lean transform in `normalize-row`. |
...
- **EAS-PLAN-001.CM2:** Cutover is one explicit trusted migrate op run per existing world, including the canonical coordination world, which carries real data (~2k strands, 4.67MB attribute bytes). The op reads each `strands.attributes` document, writes one row per key (all `archived = 0`), verifies row/document parity, and is safe to re-run on an already-migrated world (`EAS-DELTA-003.D2`).
...
- **EAS-PLAN-001.CM5:** `archive!`/`unarchive!`/migrate are trusted Clojure surface only — no socket op, no `strand` command (TEN-006). Core ships no archive policy.
...
- **EAS-PLAN-001.Q1:** The exact secondary-index set, the `archive!`/`unarchive!`/migrate op names and arg/result schemas, and the self-join/assembly SQL shapes are implementation mechanics finalized in the phases above and pinned by the benchmark gate and uniform-capability tests (`EAS-DELTA-001.Q1`–`Q4`, `EAS-DELTA-003.Q1`), preserving the delta behavior.
...
- **EAS-PLAN-001.TC5:** Archive is a flag, not a table or a second concept. Ship `archive!`/`unarchive!` as trusted primitives only; ship no archive policy and no CLI command.
- **EAS-PLAN-001.TC6:** Cutover is explicit and once: fail loud on the document schema, migrate via the one trusted op with parity verification. Never auto-migrate on open (TEN-000, TEN-003).
