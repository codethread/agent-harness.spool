...
- **EAS-DELTA-002.CC3:** SPEC-002.C21's "no public command declares or lists indexed attribute keys" and SPEC-002.C40's "storage hot-key declaration" mention (SPEC-002.C21 lists it among trusted-only operations) become moot on merge: the indexed-attribute-key concept is removed entirely (`EAS-DELTA-001.CC9`), so the retired clause language drops rather than being reworded. No CLI behavior changes — the concept was already command-less; the surface it described no longer exists.
