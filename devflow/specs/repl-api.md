...
A spool's effective root may carry an optional `spool.edn` manifest: a map with only the optional keys `:coordinate` (symbol; must equal the approving coordinate), `:provides` (vector or set of namespace symbols the spool exposes), `:needs` (map of coordinate symbol to `nil` or `{:suggest {:git/url <non-blank string>}}` — a fulfillment hint, never resolution), and `:docs` (string, or map of namespace symbol to string). Unknown keys or malformed shapes fail that spool's sync loudly (`:manifest-invalid`) without loading the root. There is no version field: the pinned sha is the content/behavior contract, and compatibility claims are limited to mechanically verifiable namespace presence. Successful sync results include the normalized manifest under `:manifest` and, when declared needs are not satisfied by approved successfully-synced coordinates, `:unmet-needs` data; needs never trigger fetching. `use!` gates activation on this state with skip reasons `:unmet-needs` and `:provides-unloadable` (each thrown under `:required? true`), checked after approval/sync and before `:after` (SPEC-004.C93–C94).

Helpers include:

- `(runtime-alpha/approved runtime)` returns normalized approved config.
- `(runtime-alpha/sync! runtime)` materializes approved git coordinates into the content-addressed cache, uses Clojure runtime dependency tooling to add each entry's effective root, and returns structured results for loaded, already-available, and failed spools including fetch, manifest, and unmet-needs data.
