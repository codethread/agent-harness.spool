  ...
  ([_rt attributes]
   ...
   (require-valid! ::harness/harness-definition
                   {:modes #{:headless :interactive}
                    :prepare 'ct.spools.codex-harness/prepare
                    :finish 'ct.spools.codex-harness/finish
...
(defn prepare
  "Turn the resolved harness and full run strand into Codex argv."
  ...
  (let [options (prepare-options run)]
    ...
    (require-valid! ::argv (codex-command options)
                    "Codex prepare produced invalid argv")))
