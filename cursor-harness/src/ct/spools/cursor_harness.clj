  ...
  ([_rt attributes]
   ...
   (require-valid! ::harness/harness-definition
                   {:modes #{:headless :interactive}
                    :prepare 'ct.spools.cursor-harness/prepare
                    :finish 'ct.spools.cursor-harness/finish
...
(defn prepare
  "Turn the resolved harness and full run strand into Cursor argv."
  ...
  (let [options (prepare-options run)]
    ...
    (require-valid! ::argv (cursor-command options)
                    "Cursor prepare produced invalid argv")))
