  ...
  ([_rt attributes]
   ...
   (require-valid! ::harness/harness-definition
                   {:modes #{:headless :interactive}
                    :prepare 'ct.spools.pi-harness/prepare
                    :finish 'ct.spools.pi-harness/finish
...
(defn prepare
  "Turn the resolved harness and full run strand into Pi argv."
  ...
  (let [options (prepare-options run)]
    ...
    (require-valid! ::argv (pi-command options)
                    "Pi prepare produced invalid argv")))
...
(defn- pi-command [{:keys [mode resumes session-id prompt] :as options}]
