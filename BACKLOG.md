...
- [ ] `lxwfq` Shuttle crash-recovery race: install-time scan (and event-handler scans in the shuttle-install→config window) respawns recovered runs before config registers harness aliases, permanently failing them (Harness not found :build; runs wcoz8/ogxf8 2026-07-04). Full diagnosis + fix options in notes on strand bvrx8. Preferred fix: recovery-origin unknown-harness defers run to pending for later scans instead of terminal failed, keeping user-created unknown-harness spawns loud (TEN-003); add regression test for alias-registered-after-install recovery (gap: shuttle_test.clj covers reconcile respawn and loud missing-harness, not their interaction).
- [ ] `b4nml` Recon/review runs spawned with --for <task> block later 'agent delegate' on that task ('task already has a successful run'); read-only runs arguably should not count as serving the task — decide serving semantics or add a delegation override. Observed 2026-07-04 during post-RFC-019 fan-out (coordination strand ymu6r).
- [ ] `fsibm` Test shard-B subprocess exited non-zero once (no parseable summary) on 2026-07-04, not reproduced across two immediate re-runs; fold into the test-concurrency flake evidence and consider capturing shard stderr on non-zero exit for one-shot diagnosis.
