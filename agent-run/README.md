...
The declaration names a source target and world policy only. Agent-run collects backend declarations from source; harness and alias declarations are collected only when a source explicitly selects them. The `agent-run-engine` lifecycle resource owns event dispatch and crash recovery.
...
(agent-run/defharnesses tools
  ...
  {:worker {:argv ["sh"]}})
(agent-run/defaliases seats
  "Seats used by this module."
...
(agent-run/defharnesses tools "Tools" {:worker {:argv ["sh"]}})

;; After: inert declaration plus explicit selection.
(agent-run/defharnesses tools "Tools" {:worker {:argv ["sh"]}})
(agent-run/use-harnesses! tools)
```
