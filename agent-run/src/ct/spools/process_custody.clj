(ns ct.spools.process-custody
  "Shared headless process custody for the legacy and harness-core engines.

  Mill owns the child and retains its completion fact. This namespace keeps the
  small translation between durable run attributes and Mill's explicit-runtime
  process API in one place; each engine still owns its own domain state machine."
