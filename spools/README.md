...
| `skein.spools.kanban` | external repo, local sibling root (see below) | [kanban.md](https://github.com/codethread/kanban.spool/blob/main/kanban.md) | — | User-facing kanban board: feature/epic cards, refinement/pending/claimed/in_review lanes, notes and handovers via `strand kanban`. |
...
`skein.spools.kanban` is the second external spool: it lives in `codethread/kanban.spool` and requires `skein.spools.devflow` (the `kanban card` devflow join), so `.skein/init.clj` activates it after devflow with both coordinates in its `:spools` guard. The repo has no remote yet, so `.skein/spools.edn` and the test JVM (`deps.edn`) both consume it as a sibling `:local/root` checkout; once published, both coordinates switch to the same sha-pinned `:git/url`+`:git/sha` pair, exactly like devflow's.
