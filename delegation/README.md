...
`agent review --roster <name>` spawns one read-only run per seat (stamped `review/roster` for attribution) plus the synthesizer, which receives the base review contract; synthesis weighs findings roster-independently. This repository ships no named reviewer roster: harnesses and aliases are consumer-owned configuration, and a consuming workspace must register its own roster near the root of its trusted config so the review policy is one obvious document.
