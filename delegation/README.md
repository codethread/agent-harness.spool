...
`agent review --roster <name>` spawns one read-only run per seat (stamped `review/roster` for attribution) plus the synthesizer, which receives the base review contract — synthesis weighs findings roster-independently. In this repository the available harness and seat configuration lives in `.skein/config/harnesses.clj`; keep your roster near the root of your workspace config so the review policy is one obvious document.
