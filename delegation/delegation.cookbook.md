...
- **The roster is the single source of truth, so it fails loudly on drift.**
  ...
  ([`.skein/config/harnesses.clj`](../.skein/config/harnesses.clj)), not scattered flags.
...
Honest source: this repo's [`.skein/config/harnesses.clj`](../.skein/config/harnesses.clj) harness and seat configuration, and the review verb and roster semantics in [`delegation/README.md` §3](./delegation/README.md#reviewer-rosters).
